...
# Адаптер классов или объектов? Зависит от ситуации.
# В данном случае адаптеров объектов никак бы не улучшил ситуацию, т.к. интерфейс сервиса меняется полностью.
# Вообще, слабо представляю ситуацию, где удобно было бы использовать CoI с постоянно меняющимся интерфейсом.
# Ведь откуда адаптер, не знающий конкретного класса Серсива, будет знать, какие именно методы Сервиса вызывать?
# При первом же изменении в каком-то названии метода или возвращаемом типе он просто сломатеся.
